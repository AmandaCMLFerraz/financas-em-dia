/*
  ...
  - The primary key for the `transaction_types` table will be changed. If it partially fails, the table could be left without primary key constraint.
  - The `id` column on the `transaction_types` table would be dropped and recreated. This will lead to data loss if there is data in the column.
  - Changed the type of `transaction_type_id` on the `transactions` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  - Changed the type of `payment_method_id` on the `transactions` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
...
-- DropForeignKey
ALTER TABLE "transactions" DROP CONSTRAINT "transactions_transaction_type_id_fkey";
...
-- AlterTable
ALTER TABLE "transaction_types" DROP CONSTRAINT "transaction_types_pkey",
DROP COLUMN "id",
ADD COLUMN     "id" SERIAL NOT NULL,
